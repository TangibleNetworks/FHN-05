// FHN_05.ino - 28/05/2015
// Tangible Networks
// Espen (Based on FHN by Antos)
//
// Code ported to TN-05
//
...
void loop () {
  ...
  for (int i = 0; i < 6; i++) {
    if (Tn.isConnected(i)) {
      vInput = vInput + (Tn.analogRead(i) - vOld[0]);
      nConnections++;
    }
  }
  if (nConnections > 0) {
  ...
  // desired tone :)
  dipBitMap = B001*Tn.dip0() + B010*Tn.dip1();
